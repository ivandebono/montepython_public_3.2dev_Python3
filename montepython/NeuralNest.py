...
def from_NN_output_to_chains(folder):
    ...
    """
    Warning: Do not use absolute paths with ~ in folder name.
    """

### TODO
# If derived parameters present, only run info /NN on non-derived (done by default)
#
# Get list of non-derived by selecting base.paramnames[0:-n_derived]
# Then run on whole folder with only the non-derived
# Then add derived in the second info run on the whole folder
    if os.path.exists(os.path.join(folder, 'base.paramnames')):
        paramnamesfile=os.path.join(folder, 'base.paramnames')
    if os.path.exists(os.path.join(folder, 'base.arguments')):
        argumentsfile=os.path.join(folder, 'base.arguments')
